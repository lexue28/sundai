...
echo "🚀 Starting Sundai FastAPI and SQLite setup..."
...
if [ ! -f "api_server.py" ]; then
    echo "⚠️  Warning: api_server.py not found. Make sure you've uploaded all project files."
fi
...
if pgrep -f "uvicorn api_server:app" > /dev/null; then
    echo "⚠️  FastAPI server is already running. Stopping it..."
